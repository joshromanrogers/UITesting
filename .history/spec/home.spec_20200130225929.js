...
describe("Sign in flow", function() {
  ...
  it("get started tester", async () => {
    await basePage.clickButton("GET STARTED");
    const signInTitle = await driver.getTitle();
    expect(signInTitle).toBe('Sign in | Luno');
  });
  ...
  it("sends user to update password", async () => {
    ...
    let a = await driver.findElement(By.id('mat-input-0'))
    await a.sendKeys(users[0].email);
    await a.sendKeys(Key.TAB + " ");
