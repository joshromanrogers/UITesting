...
class LoginPage extends BasePage {
  ...
  async enterEmailAndSubmit() {
      ...
      this.driver.wait(webdriver.until.elementIsVisible(email), 5000).sendKeys('roman@luno.com', Key.RETURN);
  }

  async enterPasswordAndSubmit() {
    let password = driver.wait(until.elementLocated(webdriver.By.name("password")), 5000);
    this.driver.wait(webdriver.until.elementIsVisible(password), 5000).sendKeys('StagingPassword1', Key.RETURN);
  }
