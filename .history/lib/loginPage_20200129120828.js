...
class LoginPage extends BasePage {
  ...
  async enterEmailAndSubmit() {
    await driver.wait(driver.findElement(By.name("email")).then((email) => email.sendKeys('roman@luno.com', Key.RETURN), 50000));
  }

  async enterPasswordAndSubmit() {
    await driver.wait(driver.findElement(By.name('password')).then((keys) => keys.sendKeys('StagingPassword1', Key.RETURN), 100000));
  }
